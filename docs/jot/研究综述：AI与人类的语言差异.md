...
larashero3-dotcom 基于 283 万字的汉语语料统计，通过正则表达式匹配并计算频率银子，开发了一个“去 AI 味” AI Skill，并将研究报告与成果托管于 [GitHub](https://github.com/larashero3-dotcom/lieflat-less-ai-tone)。笔者认为其研究过程有失严谨性，但结论有一定参考价值。

### 研究过程

1. 数据采集：生成侧语料料由不同的 AI 提供，以此增强结论普适性；生成时不开启联网搜索，不提供写作风格指令。人类侧为公开发表文本。
2. 算子实现：通过正则表达式匹配文本模式，作者自述为“本研究方法链中最薄弱者”，“正则匹配字面形态而不解析语义”。为减少测量失误，作者先抽取 20 条命中算子的实例，检查后再人工决定是否采信。
3. 数据清洗：分句、剔除非正文、分段。人类侧文本，按照来源分组使用算子进行统计，剔除组间差异大的算子，以此减少个人文风对结论的影响。
4. 结果处理：对于每个算子，定义因子 $R=生成侧频率\div人类侧频率$ 作为主要判据。$R\lt1.25$ 视为无区分力。同时剔除不可执行的算子，如“比喻是否贴切”这类需依赖语义理解的算子。（作者自述如此，笔者不理解之处：既采用正则表达式实现算子，何来需依赖语义理解者？）
...
作者也统计了不同 AI 模型间的测量结果，认为差异很大且不可忽略。例如 DeepSeek 偏好破折号，GPT 偏好对举结构。作者由此引出两条结论：
