...
1. 不在 h2 标题中使用行内代码块。
2. 不同时使用超链接和加粗，比如 **[这样](/)**。
3. 使用行内代码块的场合：代码片段、类型/字段名、枚举值、文件名/扩展名、有特殊意义的字面量。
4. 非必要不使用 Docusaurus 提供的[告示](docusaurus%E7%9A%84mdx%E7%89%B9%E6%80%A7#%E5%91%8A%E7%A4%BA)特性，可以使用引用块替代。鉴于 note 和 info 的语义可能存在混淆，下作区分：
   1. quote：引用的文字、相对不重要的信息、扩展信息、碎碎念；
   2. note：相对不重要的信息，但比 quote 更需要引起注意；
   3. info：重要补充说明，注意力等级最高（低于 warn、danger）；
   4. footnote：最不重要的信息，不应影响正文展开。
...
- 把 `<details>` 改成颜色更淡的样式（蓝色 $\rightarrow$ 灰色），减小了视觉冲击。
